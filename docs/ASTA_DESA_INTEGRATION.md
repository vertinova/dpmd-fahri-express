...
| Berkas foto di `storage/Survey/...` | URL-nya sah dan berkasnya sehat, tetapi domainnya di belakang Cloudflare dengan **hotlink protection menyala**: permintaan yang membawa `Referer` dari domain lain dibalas **403**. Diperiksa langsung — URL yang sama balas 200 tanpa `Referer` dan 403 dengan `Referer: https://dpmd.bogorkab.go.id`. Karena itu setiap `<img>` yang menampilkan foto ASTA DESA harus memasang `referrerPolicy="no-referrer"`; tanpa itu fotonya selalu tampil sebagai gambar rusak meski semua data benar |
